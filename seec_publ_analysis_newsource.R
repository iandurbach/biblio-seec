...
seec_pubs <- seec_pubs %>% 
  left_join(seec_core_pubs_urc %>% select(short_title, author, credits), by = c("short_title", "author"))
...
seec_pubs <- seec_pubs %>%
  mutate(credited = (short_title %in% unique(seec_core_pubs_urc$short_title)),
         best_match = closest_match(title, unique(seec_core_pubs_urc$title)),
...
ggsave("output/sul_pub_counts_core.png", p1, width=6, height=4, dpi = 300)
ggsave("output/sul_units_counts_core.png", p2, width=6, height=4, dpi = 300)
ggsave("output/sul_units_subs_core.png", p2a, width=6, height=4, dpi = 300)
ggsave("output/sul_journal_counts_core.png", p3, width=6, height=12, dpi = 300)
...
set.seed(100)
png("output/sul_collab_network_core.png", width=12, height=8, units = "in", res = 300)
...
# only keep papers by seec members
seec_pubs <- pubs %>% 
  mutate(name_year = paste(author, year, sep = "_")) %>%
...
# add in credits earned and verified by UCT
seec_pubs <- seec_pubs %>% 
  left_join(seec_allaff_pubs_urc %>% select(short_title, author, credits), by = c("short_title", "author"))
...
seec_pubs <- seec_pubs %>%
  mutate(credited = (short_title %in% unique(seec_allaff_pubs_urc$short_title)),
         best_match = closest_match(title, unique(seec_allaff_pubs_urc$title)),
         match = (title == best_match))
seec_pubs <- seec_pubs %>% select(title, best_match, match, credited, credits, everything()) %>% arrange(desc(year), title)
...
ggsave("output/sul_pub_counts_allaff.png", p1, width=6, height=4, dpi = 300)
ggsave("output/sul_units_counts_allaff.png", p2, width=6, height=4, dpi = 300)
ggsave("output/sul_units_subs_allaff.png", p2a, width=6, height=4, dpi = 300)
ggsave("output/sul_journal_counts_allaff.png", p3, width=6, height=12, dpi = 300)
...
author_projection <- bipartite.projection(g)$proj2
set.seed(100)
png("output/sul_collab_network_allaff.png", width=12, height=8, units = "in", res = 300)
plot.igraph(author_projection, edge.width = 5, edge.color = "black")
dev.off()
